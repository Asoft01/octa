...
*/
   ...
   var timeout = null;
	$("#ac-library-menu").hover(function() {
        timeout = setTimeout(function(){

            var $this = $(".header-container");
	    $this.addClass('active-library-icon-pro');
        }, 400);
    },
    function(){
	...
	$( "#library-header" ).mouseleave(function(e) {
		if(e.target.id == 'library-header') {
			$('.select2tag').select2('close');
			var $this = $(".header-container");
			$this.removeClass('active-library-icon-pro').addClass('hide-library-icon-pro');
		}
	  });

	$( "#ac-live-menu" ).hover(function() {
		var $this = $(".header-container");
		if ($this.hasClass('active-library-icon-pro')) {
			$this.removeClass('active-library-icon-pro').addClass('hide-library-icon-pro');
		}
	  });

	  $( "#ac-logo" ).hover(function() {
		var $this = $(".header-container");
		if ($this.hasClass('active-library-icon-pro')) {
			$this.removeClass('active-library-icon-pro').addClass('hide-library-icon-pro');
		}
	...
	$("#ac-header-search-icon").on('click', function(e){
		var $this = $(".header-container");
	...
	$(document).on('click', function(e){
	    ...
	    if (e.target.id != 'ac-header-search-icon' && !$('#ac-header-search-icon').find(e.target).length) {
			...
			if (e.target.id != 'library-header' && !$('#library-header').find(e.target).length) {
				if(e.target.id == 'select2-tags-container') {
					if ($(".header-container").hasClass('active-library-icon-pro')) {
						$(".header-container").removeClass('active-library-icon-pro').addClass('hide-library-icon-pro');
					}
				}
			}
...
=============================================== 07. Carousel JS  ===============================================
*/	
	$('.allprogression-video-carousel').owlCarousel({
	    margin:12,
	    items:4,
		autoplay:false,
		lazyLoad:true,
...
*/	
	$('.tpshare').tooltipster({
		...
		interactive: 'true',
		autoClose: 'false',
		trigger: ('ontouchstart' in window) ? 'click' : 'click'
	});	
